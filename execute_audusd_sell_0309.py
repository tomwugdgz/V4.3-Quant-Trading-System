...
print("=" * 80)
print("EXECUTE AUDUSD SELL - 信号强度 0.309% (达标！)")
print("=" * 80)
...
entry_price = tick.bid  # SELL 用 bid 价
...
if result.retcode == 10009:
    ...
    try:
        ...
        VALUES (?, ?, ?, ?, ?, ?, ?, ?)
        ''', (result.order, symbol, "SELL", lot_size, result.price, sl_price, tp_price, datetime.now()))
        conn.commit()
        conn.close()
        print("[OK] 已记录到数据库")
...
if positions:
    for pos in positions:
        print(f"  - {pos.volume:.2f} 手 @ {pos.price_open:.5f}, 盈亏：${pos.profit:.2f}")
