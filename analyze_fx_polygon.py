...
# Polygon.io API key
API_KEY = os.getenv('API_KEY_POLYGON', "nHnLvRQLvTkMfkmA4jawK68ygftRMjSU")
...
def get_daily_data(ticker, days=180):
    # Polygon format: C:EURUSD for forex
    url = f"https://api.polygon.io/v2/aggs/ticker/C:{ticker}/range/1/day/{days}daysAgo/now?apiKey={API_KEY}"
...
def analyze_symbol(symbol, name):
    ...
    data = get_daily_data(symbol, 180)
    ...
    if symbol in ["EURUSD", "GBPUSD", "AUDUSD", "USDJPY", "USDCAD"]:
