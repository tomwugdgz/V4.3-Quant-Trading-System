...
API_KEY = os.getenv('API_KEY_ALPHAVANTAGE', "HGTBD9DSFEHLBUAF")
...
def get_daily_data(from_currency, to_currency):
...
# Analyze a symbol
def analyze_symbol(pair, name):
    from_curr, to_curr = pair.split('/')
    print(f"\n{'='*60}")
    print(f"分析: {name} ({pair})")
    print(f"{'='*60}")
    
    data = get_daily_data(from_curr, to_curr)
    ...
    return {
        ...
        "direction": direction,
        "atr": atr,
        "sl": sl if direction else None,
        "tp": tp if direction else None,
...
def main():
    ...
    results = []
    for pair, name in symbols:
        res = analyze_symbol(pair, name)
        if res:
            results.append(res)
    ...
    opportunities = [r for r in results if "[OK]" in r['suggestion']]
    if opportunities:
        for opp in opportunities:
            print(f"\n{opp['name']}: {opp['trend']} - {opp['suggestion']}")
            print(f"  {opp['direction']} @ {opp['current']:.5f} | SL: {opp['sl']:.5f} | TP: {opp['tp']:.5f}")
    else:
        print("\n当前没有明确的高概率机会，建议耐心等待。")
